...
Used by: domain-referee.md, methods-referee.md (via $clo-review --peer [journal])
-->

## How This Works

When `$clo-review --peer [journal]` is invoked:
...
4. **Profile NOT found** -> referees use the journal name + ~/.codex/skills/clo-workflow/references/domain-profile.md  to adapt (still better than generic)
...
### Regional Studies
**Focus:** Regional and urban development, place-based policy, spatial inequality, regional labor markets, infrastructure, and local resilience.
**Bar:** Strong regional question, credible identification, and a clear mechanism or policy implication that speaks to spatial adjustment.
**Domain referee adjusts:** Expects a sharp place-based contribution, institutional detail about the geography, and a clear link from the local shock to regional adjustment margins.
**Methods referee adjusts:** Wants spillovers handled carefully, geography justified, mapping or spatial evidence clear, and sensitivity to alternative distance bands or regional definitions.
**Typical concerns:** "Why this spatial unit?" "Are these effects just displacement across nearby places?" "How general is this beyond your setting?" "What does this imply for place-based policy?"
**Referee pool:** POLICY (high), MEASUREMENT (medium), CREDIBILITY (medium), STRUCTURAL (low), THEORY (low), SKEPTIC (low)

### Journal of Economic Geography (JEG)
**Focus:** Economic geography, spatial development, agglomeration, conflict, trade, and how location shapes economic behavior.
**Bar:** Clear contribution to economic geography with strong spatial measurement, persuasive identification, and mechanism evidence that teaches something beyond a single case.
**Domain referee adjusts:** Expects engagement with economic geography debates, spatial equilibrium intuition, and heterogeneity that clarifies why location matters.
**Methods referee adjusts:** Scrutinizes spatial measurement, treatment geography, boundary definitions, spillovers, sorting, and robustness to alternative spatial constructions.
**Typical concerns:** "What is the spatial mechanism?" "Why should economic geographers care beyond this case?" "How sensitive are the results to geographic definitions?" "Are spillovers and selection separated cleanly?"
**Referee pool:** STRUCTURAL (medium), CREDIBILITY (medium), MEASUREMENT (medium), POLICY (medium), THEORY (low), SKEPTIC (low)
